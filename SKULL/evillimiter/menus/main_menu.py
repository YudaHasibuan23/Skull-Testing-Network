...
class MainMenu(CommandMenu):
    ...
    def _help_handler(self, args):
        ...
        IO.print(
            """
{y}scan (--range [rentang IP]){r{}memindai host online di jaringan Anda.
{s}diperlukan untuk menemukan host yang ingin Anda batasi.
{b}{s}e.g.: scan
{s}      scan --range 192.168.178.1-192.168.178.50
{s}      scan --range 192.168.178.1/24{r}

{y}host (--force){r{}mencantumkan semua host yang dipindai.
{s}berisi informasi host, termasuk ID.

{y}limit [ID1,ID2,...] [rate]{r}{}membatasi bandwidth host (uload/dload).
{y}      (--upload) (--download){r}{}{b}e.g.: limit 4 100kbit
{s}      limit 2,3,4 1gbit --download
{s}      limit all 200kbit --upload{r}

{y}block [ID1,ID2,...]{r{}memblokir akses internet host.
{y}      (--upload) (--download){r}{}{b}e.g.: block 3,2
{s}      block all --upload{r}

{y}free [ID1,ID2,...]{r}{}membatalkan batas/membuka blokir host.
{b}{s}e.g.: free 3
{s}      free all{r}

{y}add [IP] (--mac [MAC]){r}{}menambahkan host khusus ke daftar host.
{s}mac resolved automatically.
{b}{s}e.g.: add 192.168.178.24
{s}      add 192.168.1.50 --mac 1c:fc:bc:2d:a6:37{r}

{y}monitor (--interval [time in ms]){r}{}memonitor penggunaan bandwidth dari host terbatas.
{b}{s}e.g.: monitor --interval 600{r}

{y}analyze [ID1,ID2,...]{r}{}menganalisis lalu lintas host tanpa batasan
{y}        (--duration [time in s]){r}{}to menentukan siapa yang menggunakan berapa banyak bandwidth.
{b}{s}e.g.: analyze 2,3 --duration 120{r}

{y}watch{r}{}mendeteksi koneksi ulang host dengan IP yang berbeda.
{y}watch add [ID1,ID2,...]{r}{}menambahkan host ke daftar pantauan koneksi ulang.
{b}{s}e.g.: watch add 3,4{r}
{y}watch remove [ID1,ID2,...]{r}{}menghapus host dari daftar pantauan koneksi ulang.
{b}{s}e.g.: watch remove all{r}
{y}watch set [attr] [value]{r}{}mengubah sambungkan kembali pengaturan jam tangan.
